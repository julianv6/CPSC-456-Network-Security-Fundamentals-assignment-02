...
def generateHeaderFile(execList, fileName):
	...
	for arg in execList:
		arg = getHexDump(arg)
		progLens.append(len(arg)/5);
		headerFile.write("\tnew unsigned char[" + str(len(arg)/5) + "]{" + arg[:-1] +"},\n");
	headerFile.write("};");
	...
	for progLen in progLens:
		progLenString += str(progLen) + ","
	headerFile.write(progLenString[:-1]+"};");
